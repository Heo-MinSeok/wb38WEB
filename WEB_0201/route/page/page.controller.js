  ...
  try {
    ...
    if (req.params.stunum == user.stunum) {
      if (user.stunum == 202403838)
        res.sendFile(__dirname + "/admin/main_admin.html");
      else
...
exports.GetEditinfo = (req, res) => {
  ...
  try {
    ...
    if (req.params.stunum == user.stunum) {
      if (user.stunum == 202403838) {
        res.sendFile(__dirname + '/admin/editinfo_amdin.html');
      }
      else {
...
exports.GetAllcheck = (req, res) => {
  ...
  try {
    ...
    if (req.params.stunum == user.stunum && user.stunum == 202403838) {
      res.sendFile(__dirname + '/admin/editinfo_admin.html');
    }
    else {
      res.sendFile(__dirname + '/pc/editinfo.html');
    }
...
exports.GetCheck = (req, res) => {
  ...
  try {
    ...
    if (req.params.stunum == user.stunum) {
      if (user.stunum == 202403838)
        res.sendFile(__dirname + "/admin/check_admin.html");
      else
...
exports.GetCheckList = (req, res) => {
  console.log(`GET: /main/${req.params.stunum}/checklist`);
  const { token } = req.cookies;
  try {
    const user = jwt.verify(token, config.ACCESS_TOKEN_SECRET); // 검증
    console.log(user);
    if (req.params.stunum == user.stunum) {
      if (user.stunum == 202403838)
        res.sendFile(__dirname + "/admin/check_list_admin.html");
      else
        res.sendFile(__dirname + '/pc/check_list.html');
    }
    else {
      res.sendFile(__dirname + '/pc/autherr.html');
    }
...
exports.GetAllcheck = (req, res) => {
  ...
  try {
    ...
    console.log(user);
    if (req.params.stunum == user.stunum && user.stunum == 202403838) {
      res.sendFile(__dirname + '/admin/allcheck.html');
    }
    else {
      res.sendFile(__dirname + '/pc/autherr.html');
    }
...
exports.GetFiles = (req, res) => {
  ...
  try {
    ...
    if (req.params.stunum == user.stunum) {
      if (user.stunum == 202403838)
        res.sendFile(__dirname + '/admin/file_admin.html');
      else
